...
<?php
    ...
    function edit_donor()
    {
        if(!empty($_POST['name']) && !empty($_POST['address']) && !empty($_POST['area']) && !empty($_POST['sub-area']) && !empty($_POST['branch']) && !empty($_POST['bg']) && !empty($_POST['nid']) && !empty($_POST['phone']) && !empty($_POST['email'])) 
        {
            ...
            global $e;
            $query = "UPDATE donor SET b_id ='$branch', d_name ='$name', address ='$address', area='$area', subarea ='$sub_area', national_id ='$nid', blood_group ='$bg', phone ='$phone', email ='$email' WHERE d_id =".$e;
    ...
    <form id="add-donor-form" name="donorform" action="edit-donor.php?e=<?php echo $e;?>" method="post">
       <br>
        ...
        <p id="pcat" class="form-text">Select Branch : </p>
             <select name="branch">
                 <?php
                    $conc = oci_connect('system', 'root', 'localhost/orcl');
            ...
            <p id="pcat" class="form-text">Blood Group : </p>
             <select name="bg">
                 <?php
                    $cob = oci_connect('system', 'root', 'localhost/orcl');
                
                    $var = $row['BLOOD_GROUP'];
                   $sdtt = oci_parse($cob, "SELECT * FROM blood");
                   oci_execute($sdtt);
